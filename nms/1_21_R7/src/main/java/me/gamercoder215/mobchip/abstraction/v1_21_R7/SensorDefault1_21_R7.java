...
final class SensorDefault1_21_R7 implements Sensor<LivingEntity> {

    private final net.minecraft.world.entity.ai.sensing.Sensor<?> handle;

    public SensorDefault1_21_R7(net.minecraft.world.entity.ai.sensing.Sensor<?> handle) {
        this.handle = handle;
    }
    ...
    public @NotNull List<Memory<?>> required() {
        return handle.requires().stream().map(ChipUtil1_21_R7::fromNMS).collect(Collectors.toList());
    }
    ...
    public void run(@NotNull World w, LivingEntity entity) {
        try {
            ...
            doTick.invoke(handle, ChipUtil1_21_R7.toNMS(w), ChipUtil1_21_R7.toNMS(entity));
        } catch (ReflectiveOperationException e) {
            Bukkit.getLogger().severe(e.getMessage());
            for (StackTraceElement st : e.getStackTrace()) Bukkit.getLogger().severe(st.toString());
        }
    ...
    public NamespacedKey getKey() {
        ...
        BuiltInRegistries.SENSOR_TYPE.stream()
                ...
                .findFirst()
                .ifPresent(s -> key.set(ChipUtil1_21_R7.fromNMS(BuiltInRegistries.SENSOR_TYPE.getKey(s))));
