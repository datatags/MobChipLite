...
final class EntitySenses1_21_R7 implements EntitySenses {
    ...
    public EntitySenses1_21_R7(Mob m) {
        this.m = m;
    ...
    public @NotNull List<Sensor<?>> getSensors() {
        return sensorsHandle.values()
                .stream()
                .map(ChipUtil1_21_R7::fromNMS)
                .collect(Collectors.toList());
    }

    @Override
    public void addSensor(@NotNull Sensor<?> sensor) throws IllegalArgumentException {
        if (!new ChipUtil1_21_R7().existsSensor(sensor.getKey())) throw new IllegalArgumentException("Unregistered Sensor: " + sensor.getKey());

        sensorsHandle.put(ChipUtil1_21_R7.toNMSType(sensor), ChipUtil1_21_R7.toNMS(sensor));
        save();
    }
    ...
    public void removeSensor(@NotNull NamespacedKey key) {
        if (!new ChipUtil1_21_R7().existsSensor(key)) throw new IllegalArgumentException("Unregistered Sensor: " + key);

        Identifier keyH = ChipUtil1_21_R7.toNMS(key);
    ...
    public boolean hasSensor(@NotNull NamespacedKey key) {
        ...
        for (SensorType<?> t : sensorsHandle.keySet()) {
            ...
            if (ChipUtil1_21_R7.toNMS(key).equals(currentKey)) {
                b.set(true);
                break;
            }
        }
