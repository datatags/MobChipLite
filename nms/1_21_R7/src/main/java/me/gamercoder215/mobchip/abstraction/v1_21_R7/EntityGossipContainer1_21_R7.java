...
final class EntityGossipContainer1_21_R7 implements EntityGossipContainer {
    private final GossipContainer handle;
    private final Villager entity;

    public EntityGossipContainer1_21_R7(Villager v) {
        this.entity = v;
    ...
    public int getReputation(@NotNull Entity en, @Nullable GossipType... types) throws IllegalArgumentException {
        return handle.getReputation(en.getUniqueId(), g -> Arrays.asList(types).contains(ChipUtil1_21_R7.fromNMS(g)));
    }

    @Override
    public void put(@NotNull Entity en, @NotNull GossipType type, int maxCap) throws IllegalArgumentException {
        handle.add(en.getUniqueId(), ChipUtil1_21_R7.toNMS(type), maxCap, Villager.ReputationEvent.UNSPECIFIED);
    }

    @Override
    public void remove(@NotNull Entity en, @NotNull GossipType type) throws IllegalArgumentException {
        handle.remove(en.getUniqueId(), ChipUtil1_21_R7.toNMS(type), Villager.ReputationEvent.UNSPECIFIED);
    }

    @Override
    public void removeAll(@NotNull GossipType type) throws IllegalArgumentException {
        handle.remove(ChipUtil1_21_R7.toNMS(type), Villager.ReputationEvent.UNSPECIFIED);
    }
}
