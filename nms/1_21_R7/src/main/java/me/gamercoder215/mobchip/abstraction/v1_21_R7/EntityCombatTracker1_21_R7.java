...
final class EntityCombatTracker1_21_R7 implements EntityCombatTracker {

    private final CombatTracker handle;
    private final Mob m;

    public EntityCombatTracker1_21_R7(Mob m) {
        this.m = m;
        this.handle = ChipUtil1_21_R7.toNMS(m).getCombatTracker();
    }
    ...
    public @Nullable CombatEntry getLatestEntry() {
        try {
            ...
            return last == null ? null : ChipUtil1_21_R7.fromNMS(m, last);
    ...
    public @NotNull List<CombatEntry> getCombatEntries() {
        ...
        try {
            ...
            ((List<net.minecraft.world.damagesource.CombatEntry>) f.get(handle)).stream().map(en -> ChipUtil1_21_R7.fromNMS(m, en)).forEach(entries::add);
        } catch (Exception e) {
            ChipUtil.printStackTrace(e);
        }
        return entries;
    ...
    public void recordEntry(@NotNull CombatEntry entry) {
        ...
        try {
            ...
            m.invoke(entries, ChipUtil1_21_R7.toNMS(entry));
        } catch (Exception e) {
            ChipUtil.printStackTrace(e);
        }
    }
    ...
    public boolean hasLastDamageCancelled() {
        return ChipUtil1_21_R7.toNMS(m).lastDamageCancelled;
    }
}
