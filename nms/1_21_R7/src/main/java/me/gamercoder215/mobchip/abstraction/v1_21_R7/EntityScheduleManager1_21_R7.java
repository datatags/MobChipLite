...
final class EntityScheduleManager1_21_R7 implements EntityScheduleManager {

    private final net.minecraft.world.entity.Mob nmsMob;

    public EntityScheduleManager1_21_R7(Mob m) {
        this.nmsMob = ChipUtil1_21_R7.toNMS(m);
    }
    ...
    public @NotNull Set<Activity> getActiveActivities() {
        return nmsMob.getBrain().getActiveActivities().stream().map(ChipUtil1_21_R7::fromNMS).collect(Collectors.toSet());
    }

    @Override
    public void setDefaultActivity(@NotNull Activity a) {
        nmsMob.getBrain().setDefaultActivity(ChipUtil1_21_R7.toNMS(a));
    }
    ...
    public void setRunningActivity(@NotNull Activity a) {
        nmsMob.getBrain().setActiveActivityIfPossible(ChipUtil1_21_R7.toNMS(a));
    }

    @Override
    public @Nullable Activity getRunningActivity() {
        return nmsMob.getBrain().getActiveNonCoreActivity().isPresent() ? ChipUtil1_21_R7.fromNMS(nmsMob.getBrain().getActiveNonCoreActivity().get()) : null;
    }

    @Override
    public boolean isRunning(@NotNull Activity a) {
        return nmsMob.getBrain().isActive(ChipUtil1_21_R7.toNMS(a));
    }
    ...
    public Consumer<Mob> put(@NotNull Activity key, Consumer<Mob> value) {
        nmsMob.getBrain().addActivity(ChipUtil1_21_R7.toNMS(key), 0, ImmutableList.of(ChipUtil1_21_R7.toNMS(value)));
        return value;
    }
