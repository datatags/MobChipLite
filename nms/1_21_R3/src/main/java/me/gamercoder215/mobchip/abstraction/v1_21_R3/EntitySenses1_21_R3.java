...
final class EntitySenses1_21_R3 implements EntitySenses {
    ...
    public @NotNull List<Sensor<?>> getSensors() {
        return sensorsHandle.values()
                .stream()
                .map(ChipUtil1_21_R3::fromNMS)
                .collect(Collectors.toList());
    }

    @Override
    public void addSensor(@NotNull Sensor<?> sensor) throws IllegalArgumentException {
        if (!new ChipUtil1_21_R3().existsSensor(sensor.getKey())) throw new IllegalArgumentException("Unregistered Sensor: " + sensor.getKey());

        sensorsHandle.put(ChipUtil1_21_R3.toNMSType(sensor), ChipUtil1_21_R3.toNMS(sensor));
        save();
    }
    ...
    public void removeSensor(@NotNull NamespacedKey key) {
        if (!new ChipUtil1_21_R3().existsSensor(key)) throw new IllegalArgumentException("Unregistered Sensor: " + key);

        ResourceLocation keyH = ChipUtil1_21_R3.toNMS(key);
