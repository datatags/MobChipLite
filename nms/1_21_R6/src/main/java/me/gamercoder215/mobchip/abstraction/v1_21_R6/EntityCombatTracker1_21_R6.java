...
final class EntityCombatTracker1_21_R6 implements EntityCombatTracker {
    ...
    public EntityCombatTracker1_21_R6(Mob m) {
        this.m = m;
        this.handle = ChipUtil1_21_R6.toNMS(m).getCombatTracker();
    }
    ...
    public @Nullable CombatEntry getLatestEntry() {
        try {
            ...
            return last == null ? null : ChipUtil1_21_R6.fromNMS(m, last);
    ...
    public @NotNull List<CombatEntry> getCombatEntries() {
        ...
        try {
            ...
            ((List<net.minecraft.world.damagesource.CombatEntry>) f.get(handle)).stream().map(en -> ChipUtil1_21_R6.fromNMS(m, en)).forEach(entries::add);
        } catch (Exception e) {
            ChipUtil.printStackTrace(e);
        }
        return entries;
    ...
    public boolean hasLastDamageCancelled() {
        return ChipUtil1_21_R6.toNMS(m).lastDamageCancelled;
    }
}
