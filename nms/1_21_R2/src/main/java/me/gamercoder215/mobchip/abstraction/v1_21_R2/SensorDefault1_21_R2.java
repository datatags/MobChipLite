...
final class SensorDefault1_21_R2 implements Sensor<LivingEntity> {
    ...
    public @NotNull List<Memory<?>> required() {
        return handle.requires().stream().map(ChipUtil1_21_R2::fromNMS).collect(Collectors.toList());
    }
    ...
    public void run(@NotNull World w, LivingEntity entity) {
        try {
            ...
            doTick.invoke(handle, ChipUtil1_21_R2.toNMS(w), ChipUtil1_21_R2.toNMS(entity));
        } catch (ReflectiveOperationException e) {
            Bukkit.getLogger().severe(e.getMessage());
            for (StackTraceElement st : e.getStackTrace()) Bukkit.getLogger().severe(st.toString());
        }
    ...
    public NamespacedKey getKey() {
        ...
        BuiltInRegistries.SENSOR_TYPE.stream()
                ...
                .findFirst()
                .ifPresent(s -> key.set(ChipUtil1_21_R2.fromNMS(BuiltInRegistries.SENSOR_TYPE.getKey(s))));
